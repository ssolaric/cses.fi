...
// In a 7x7 grid, how many paths are there from the upper-left square to the lower-right square?
// Each path should traverse each square once.
// ans = 111712

// NOTE: compile this with optimizations:
// rustc -C opt-level=3 16-book.rs
...
fn main() {
    ...
    // Optimization 1
    visited[0][0] = true;
    let ans = 2 * count_paths(1, 1, 0, &mut visited);
    println!("{}", ans);
}
